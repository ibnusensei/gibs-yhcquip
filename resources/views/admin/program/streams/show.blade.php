<x-app-layout>
    ...
    <div class="content container-fluid">
        ...
        <div class="card">
            <div class="card-body">
                <h4 class="card-title">{{ $streams->name }}</h4>
                <p class="card-text">{{ $streams->description }}</p>

                {{-- media --}}
                <div class="row ">
                    @foreach ($streams->getMedia('images') as $image)
                        <div class="col-md-4 mb-4">
                            <div class="img-custom rounded mb-2">
                                <img src="{{ $image->getUrl() }}" class="" alt="">
                            </div>
                            <form action="{{ route('admin.streams.image.destroy', $image) }}" method="POST" class="d-inline">
                                @method('DELETE')
                                @csrf
